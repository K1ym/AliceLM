...
#### 2.4.2 ControlPlane（配置中心）✅ 已实现

- **AliceControlPlane**：统一入口，管理模型/Prompt/工具/服务的生命周期。
- 四大组件：
  - **ModelRegistry**：管理 LLM/Embedding/ASR profiles + 任务默认映射
  - **PromptStore**：管理 Prompt 文本 + 支持用户覆盖 + 老 key 兼容
  - **ToolRegistry**：管理工具定义 + 场景默认工具集
  - **ServiceFactory**：管理 Provider 实现配置
- 配置文件：`config/models.yaml`, `config/prompts.yaml`, `config/tools.yaml`, `config/services.yaml`
- HTTP API：`/api/v1/control-plane/*` 5 个只读端点
